...
class ReceiveResults extends StatefulWidget {
  const ReceiveResults({Key? key}) : super(key: key);

  @override
  _ReceiveResultsState createState() => _ReceiveResultsState();
}

class _ReceiveResultsState extends State<ReceiveResults> {
  ReceiveResultsModel _receiveResultsState = ReceiveResultsModel();

  Future _getReceiveResults() async {
    await ConnectAPI().get('receive-results').then((value) {
      if (value.statusCode == 200 && jsonDecode(value.body)['status'] == true) {
        setState(() {
          _receiveResultsState = receiveResultsModelFromJson(value.body);
        });
      }
      //this.setDataAddress();
    });
        ...
        child: Padding(
          ...
          child: Column(
            children: [
              ...
              Mysutitle(
                text: 'แบบสรุปผลการพิจารณาอนุมัติการขอยืมอุปกรณ์ฯ ทก.11',
                vertical: 5,
              ),
              Column(
                children: _receiveResultsState.data?.map((e) {
                  int index = _receiveResultsState.data!.indexOf(e) + 1;
                  return MyCardItem(
                    no: '$index',
                    number: '${e.id ?? ''}',
                    name:
