...
class ConnectAPI{
  String url = 'http://103.74.255.182/api/';
  ...
  var haerder = {HttpHeaders.contentTypeHeader:"application/json"};
  ...
  Future<http.Response> post(String data,String path)async{
    try{
      http.Response response = await http.post(Uri.parse(url+path),body: data,headers: haerder);
  ...
  Future<http.Response> posthaerder(String data,String path)async{
    try{
      await gettoken();
      http.Response response = await http.post(Uri.parse(url+path),body: data,headers: haerder);
      return response;
  ...
  Future<http.Response> deletehaerder(String data,String path)async{
    try{
      await gettoken();
      http.Response response = await http.delete(Uri.parse(url+path),body: data,headers: haerder);
      return response;
    }catch(err){
      print(err);
      return throw Exception('Not connect Network');
  ...
  Future<http.Response> get(String path)async{
    try{
      await gettoken();
      http.Response response = await http.get(Uri.parse(url+path),headers: haerder);
  ...
  gettoken()async{
    //SharedPreferences preferences = await SharedPreferences.getInstance();
      haerder[HttpHeaders.authorizationHeader] = 'Bearer '+token;
      //print(haerder);
    //return '';
  }
